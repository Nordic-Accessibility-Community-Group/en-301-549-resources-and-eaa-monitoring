...
## Country selection and update watcher

The sequential country rollout is retired. A merged PR does not authorize starting another country. New country research requires an explicit contributor request; check current main, open PRs and pending branches before starting. Keep at most one country research PR open and never merge on the contributor's behalf.

The update-watcher pilot covers existing Denmark, Sweden, Ireland, Italy and Portugal records in `erikgustafsson/en-301-549-resources-and-eaa-monitoring-testing`. Follow [watcher/README.md](watcher/README.md) for its inventory, partial baseline, weekly source checks, monthly discovery, state branch and candidate review. Do not write to the original repository during this pilot or add jurisdictions automatically. GitHub Actions remain outside scope.

Keep country update PRs titled `Audit <country> EAA authorities and reporting routes` so the existing separate post-draft check can recognize them. Infrastructure and operational watcher files stay under `.github/agents/`; country assertions belong in separate one-country PRs. Watcher source inventories and observation snapshots are operational records, not replacements for country research or verified table claims. Ordinary evidence, isolated language and local validation gates still apply to factual updates.
